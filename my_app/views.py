...
from .serializer import show_data_serializer
...
def data_display(request):
    if request.method=='GET':
        data1=displaydata1.objects.all()
        serializer=show_data_serializer(data1,many=True)
        return Response({'mydata':serializer.data})
    if request.method == 'POST':
        serializer = show_data_serializer(data=request.data)
    if serializer.is_valid():
            serializer.save()
            return Response({'datamy': serializer.data}, status=status.HTTP_201_CREATED)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
